...
**切节点**：设置页里点「节点列表」，弹窗里能看到各分组的节点、延迟、当前选中，
点一下即切换，还有一个「测速」按钮。这个界面是 dashboard 自己的，
内核 API 只由 dashboard 在服务端调用（拿 `~/.webclaw/proxy/secret` 直连回环的 10013），
浏览器不碰内核——所以镜像里不再塞 metacubexd 那类第三方面板：
它们的 API 地址只能按 `hostname:port` 拼，挂不到 20000 的 `/proxy/10013` 路径前缀下。
